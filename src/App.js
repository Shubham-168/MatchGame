...
const randomValue = imagesList[Math.ceil(Math.random() * imagesList.length - 1)]

const initialStateValues = {
  score: 0,
  time: 60,
  activeTabId: tabsList[0].tabId,
  activeMatchedList: randomValue,
  showScorecard: false,
}
...
class App extends Component {
  ...
  onClickThumbnail = id => {
    const {score, activeMatchedList} = this.state

    if (activeMatchedList.id === id) {
  ...
  renderGameView = filteredList => {
    const {activeMatchedList} = this.state

    return (
      <>
        <ul>
          {activeMatchedList.map(eachImage => (
            <Matched key={eachImage.id} imageDetails={eachImage} />
          ))}
        </ul>
  ...
  render() {
    const {score, time, activeTabId, showScorecard} = this.state
    ...
    return (
      <div>
        ...
        {showScorecard
          ...
          : this.renderGameView(filteredList)}
      </div>
    )
  }
}
